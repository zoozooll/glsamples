...
extern "C" {
	...
	void initCreateBodys()
	{
...

		{
			angle=0.005f;
			m_nums = 20;
			tempV = new btVector3[m_nums];
			float sizeAngle = 360.0f / m_nums;
			for(int i = 0; i<m_nums; i++)
			{

				if(i%2 == 0)
				{
					tempV[i].setValue(0,0,0);//设置光线的起点
				}
				else
				{
					tempV[i].setValue(50,-30,0);//设置光线的终点
					tempV[i] = tempV[i].rotate(btVector3(0,1,0),sizeAngle*i);//更改光线的终点
				}
			}

			m_mrc = new MyRaycast(
					tempV, //顶点数组   顶点顺序有要求：第一点和第二点是一组，构成一条线。第三点和第四点是一组，构成一条线
					m_nums, //顶点数组长度		必须为偶数
					m_dynamicsWorld//物理世界指针
					);
	...
	void renderFrame() {
		...
		for(int i = 0; i<m_nums; i++)
		{
			if(i%2 != 0)
			{
				tempV[i] = tempV[i].rotate(btVector3(0,1,0),angle);//更改光线的终点坐标
			}
		}
