...
extern "C" {
	...
	void initWorld()
	{
		m_collisionConfiguration = new btDefaultCollisionConfiguration();//创建默认的碰撞检测配置信息对象BulletMultiThreaded)
	...
	void initCreateBodys()
	{
		...
		}
		{
			...
			tbTemp->getBody()->setFlags(BT_ENABLE_GYROPSCOPIC_FORCE);
			tca.push_back(tbTemp);
