...
class MySurfaceView extends GLSurfaceView 
{
	...
	private class SceneRenderer implements GLSurfaceView.Renderer 
    {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            aList.add(new RigidBody(ch,true,new Vector3f(-14f,0f,0),new Vector3f(0,0,0)));//生成左侧的刚体并放进刚体列表
            aList.add(new RigidBody(ch,true,new Vector3f(10f,0f,0),new Vector3f(0,0,0)));	//生成右侧的刚体并放进刚体列表	
            aList.add(new RigidBody(ch,false,new Vector3f(-8f,0f,0),new Vector3f(12,0,0)));	        //生成中间运动的刚体并放进刚体列表
            lgt=new LovoGoThread(aList);//创建物体运动线程对象
            lgt.start();//开启本线程
        }
    }
}