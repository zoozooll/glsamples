...
void AnyShapeByVectices::initShader()
{
	...
	muCameraHandle = glGetUniformLocation(mProgram, "uCamera");//摄像机位置属性

	maPositionHandle = glGetAttribLocation(mProgram, "aPosition");//顶点位置属性
	maTexCoorHandle = glGetAttribLocation(mProgram, "aTexCoor");//顶点纹理坐标属性
	maNormalHandle = glGetAttribLocation(mProgram, "aNormal");//顶点法向量属性
