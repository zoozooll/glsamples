	...
	void initCreateBodys()
	{
		...
		}
		{
			...
			for(int i = 0; i<3; i++)
			{
				for(int j = 0; j<5; j++)
				{
					for(int k = 0; k<3; k++)
					{
						btScalar tempX(start_x + i*2*2*UNIT_SIZE);
						btScalar tempY(start_y + j*2*2*UNIT_SIZE);
						btScalar tempZ(start_z + k*2*2*UNIT_SIZE);
						tbTemp = new TexBall(
								UNIT_SIZE*2,
								ballTexId,
								m_dynamicsWorld,
								10.0f,
								btVector3(tempX,tempY,tempZ),
								0.5f,
								0.5f);
						//将新立方体加入到列表中
						tca.push_back(tbTemp);
		...
		}
		{
			...
			tbTemp = new TexCuboid(
					...
					btVector3(4,6,0.2f),//长方体的半区域
					0.0f,//长方体的质量
					btVector3(-4,0,0),//刚体的位置
					0.0f,//恢复系数
					0.0f,//摩擦系数
					UNIT_SIZE,//单位长度
					floorId,//前侧纹理图id
			...
			tbTemp = new TexCuboid(
					m_dynamicsWorld,//物理世界对象
					btVector3(12,6,0.2f),//长方体的半区域
					0.0f,//长方体的质量
					btVector3(12-0.2,0,-12-0.2),//刚体的位置
					0.0f,//恢复系数
					0.0f,//摩擦系数
					UNIT_SIZE,//单位长度
					floorId,//wood_binId,//前侧纹理图id
					wood_binId,//后侧纹理图id
					wood_binId,//上侧纹理图id
					wood_binId,//下侧纹理图id
					floorId,//左侧纹理图id
			...
			btTransform transformTemp = tbTemp->getBody()->getWorldTransform();
			transformTemp.setRotation(btQuaternion(btVector3(0,1,0),SIMD_PI/2));
			tbTemp->getBody()->setWorldTransform(transformTemp);
			...
			transformTemp = tbTemp->getBody()->getWorldTransform();
			transformTemp.setRotation(btQuaternion(btVector3(0,1,0),-SIMD_PI/2));
			tbTemp->getBody()->setWorldTransform(transformTemp);
			...
			tbTemp = new TexCuboid(
					m_dynamicsWorld,//物理世界对象
					btVector3(12-0.4,6,0.2f),//长方体的半区域
					0.0f,//长方体的质量
					btVector3(0,0,-24),//刚体的位置
					0.0f,//恢复系数
					0.0f,//摩擦系数
					UNIT_SIZE,//单位长度
					wood_binId,//前侧纹理图id
		...
		}
		{
			...
			//btHingeConstraint* hinge1 = new btHingeConstraint(*bodyB,transformB,false);
			//参数：刚体的指针，中心点（相对于刚体位置），固定轴向量（刚体能绕哪个轴转动），false
			btHingeConstraint* hinge1 = new btHingeConstraint(*(tbTemp->getBody()),btVector3(-4,0,0.2),btVector3(0,1,0),false);//创建铰链关节
			m_dynamicsWorld->addConstraint(hinge1);//将铰链关节添加到物理世界中
