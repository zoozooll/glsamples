...
class MySurfaceView extends GLSurfaceView
{
	...
	private class SceneRenderer implements GLSurfaceView.Renderer
    {
		...
		{
			int tia[]=new int[5];//用于存放产生的帧缓冲id的数组
			GLES30.glGenFramebuffers(tia.length, tia, 0);//产生一个帧缓冲id
			frameBufferIds=tia;
			
			GLES30.glGenRenderbuffers(tia.length, tia, 0);//产生一个渲染缓冲id
			renderDepthBufferIds=tia;//将渲染缓冲id记录到成员变量中
			
			int[] tempIds = new int[5];//用于存放产生纹理id的数组
			...
			textureIds=tempIds;//将纹理id记录到成员变量
			for(int i=0;i<textureIds.length;i++)
			{
	        	...
	        	(
	        		...
	        		renderDepthBufferIds[i]			//渲染深度缓冲id
	        	);
			}
		}
		...
		public void drawSceneTexture()//绘制生成的矩形纹理
		{
			//设置视口大小及位置 
