...
public class CubeCylinder 
{
	...
	{	
		...
		localTransform.origin.set(new Vector3f(0, 0, 0));//设置变换的平移参数
		...
		comShape.addChildShape(localTransform, csa[2]);//添加子形状（围绕y轴的圆柱）
		...
		startTransform.origin.set(new Vector3f(cx, cy, cz));//设置刚体的初始位置
		...
		tc=new TexCube(halfSize,mProgram[0]);//创建绘制用立方体箱子
		cl=new Cylinder(halfSize/2,halfSize*3.6f,16,mProgram[0],mv);//创建绘制用圆柱		
		this.halfSize=halfSize;//保存半边长
	...
	public void drawSelf(int[] texIda,int[] texIdb)//绘制方法
	{
		...
		MatrixState.translate(0, -halfSize*1.8f, 0);//沿y轴进行平移
