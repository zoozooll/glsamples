...
extern "C" {
	...
	btSoftRigidDynamicsWorld*		m_dynamicsWorld;//软体物理世界类指针
	...
	void initWorld()
	{
		m_softBodyWorldInfo = btSoftBodyWorldInfo();//创建软体物理世界信息
...
//		m_softBodyWorldInfo.water_normal	=	btVector3(0,0,0);
	...
	void initCreateBodys()
	{
		...
		{
			SoftCloth* sc = new SoftCloth(m_dynamicsWorld,
					m_softBodyWorldInfo,
					btVector3(0,-5,0)
					);
			//将新立方体加入到列表中
		...
		}
		{
			tbTemp = new TexCuboid(
					...
					btVector3(UNIT_SIZE*2*0.7, UNIT_SIZE*2*0.75, UNIT_SIZE*2*0.75),//长方体的半区域
					20.0f,//长方体的质量
					btVector3(0,15,0),//刚体的位置
					0.2f,//恢复系数
					0.8f,//摩擦系数
					UNIT_SIZE,//单位长度
