...
//
btSoftBody*		btSoftBodyHelpers::CreateFromTriMesh(
		btSoftBodyWorldInfo& worldInfo,//物理世界
		const btScalar*	vertices,		//顶点数组
		const int* triangles,			//三角形索引
		int ntriangles,					//三角形个数
		bool randomizeConstraints)		//是否随机约束
{
	int	maxidx=0;
	int i,j,ni;
