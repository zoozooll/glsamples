...
public class LoadedObjectVertexNormal 
{
    ...
    public void addPickedConstraint(){
    	//创建用于拖动的点对点关节，此关节位置在被拖动物体坐标系的原点
    	p2p = new Point2PointConstraint(body, new Vector3f(0,0,0));
    	mv.dynamicsWorld.addConstraint(p2p, true);//将关节添加进物理世界
    	this.isPicked=true;//将物体的选中标志设置为true
    	
    }
    public void removePickedConstraint(){    	
    	if(p2p!=null){//若有用于拖动的关节
    		mv.dynamicsWorld.removeConstraint(p2p);//将关节从物理世界中删除
    	}
    	this.isPicked=false;//将物体的选中标志设置为false
    }
	//获得中心点位置和长宽高的方法
