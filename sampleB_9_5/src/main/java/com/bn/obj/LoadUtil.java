...
public class LoadUtil {
	...
	static float[] m_normals;
	static float[] m_texCoors;

	// 从obj文件中加载携带顶点信息的物体
	public static void loadFromFile(String fname, Resources r, MySurfaceView mv) {
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				m_vertices[i] = alvResult.get(i);
			}

			// 生成纹理数组
			size = altResult.size();
			m_texCoors = new float[size];
			for (int i = 0; i < size; i++) {
				m_texCoors[i] = altResult.get(i);
			}

			// 生成法向量数组
			size = alnResult.size();// 获取法向量列表的大小
			m_normals = new float[size];// 创建存放法向量的数组
			for (int i = 0; i < size; i++) {
				m_normals[i] = alnResult.get(i);// 将法向量值存入数组
			}
		} catch (Exception e) {
